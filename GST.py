...
class GST(nn.Module):
    ...
    def __init__(self, model_config, n_mel_channels):

        super().__init__()
        self.encoder = ReferenceEncoder(model_config, n_mel_channels)
...
class ReferenceEncoder(nn.Module):
    ...
    def __init__(self, model_config, preprocess_config):
        ...
        super().__init__()
        ref_conv_filters = model_config["gst"]["ref_conv_filters"]
        emb_dim = model_config["gst"]["E"]
        self.n_mels = preprocess_config["preprocessing"]["mel"]["n_mel_channels"]
        K = len(ref_conv_filters)
        filters = [1] + ref_conv_filters
        ...
        self.bns = nn.ModuleList([nn.BatchNorm2d(num_features=ref_conv_filters[i]) for i in range(K)])

        out_channels = self.calculate_channels(self.n_mels, 3, 2, 1, K)
        self.gru = nn.GRU(input_size=ref_conv_filters[-1] * out_channels,
...
class STL(nn.Module):
    ...
    def __init__(self, model_config):
        ...
        n_tokens = model_config["gst"]["n_style_token"]
        num_heads = model_config["gst"]["attn_head"]
        E = model_config["gst"]["E"]

        self.embed = nn.Parameter(torch.FloatTensor(n_tokens, E // num_heads))
        d_q = E // 2
        d_k = E // num_heads
